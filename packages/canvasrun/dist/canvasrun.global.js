(function () {
  ...
  class CanvasRun {
      constructor() {
          ...
          // 吧用户手机的尺寸数据转化成 750
          this.paramsTranslateUnit = (num) => {
              return parseInt(num * 750 / this.gain / document.body.clientWidth + '');
          };
          console.log('注册成功');
      }
      // 初始化
      async init({ canvas, ctx, config, gain = 1, noNeedOff = false, mounted = () => { }, created = () => { } }) {
          let lastMoveX = 0;
          this.canvas = canvas;
          this.ctx = ctx;
          this.gain = gain;
          ...
          const openAttr = (parent) => {
              ...
              openParentAttr.forEach(i => {
                  if (parent.whiteSpace === 'nowrap')
                      parent[i] = this.computedWidth(parent)[i] || parent[i];
              });
          };
      ...
      ;
      get getClassList() { return this.classList; }
      // 滑动监听，计算并设置移动距离
      ...
      ;
      dispatchEvent(events, ex, ey, type = 'click') {
      ...
      ;
      // 画圆角矩形
      drwaRoundRect({ x = 0, y = 0, w = 0, h = 0, borderRadius = 0, border = '1rpx solid transparent' }) {
      ...
      ;
      drawFont({ fontSize, color = '#000000', lineHeight, maxLine, text, textAlign = 'start', overText, overTextColor, fontWeight, x, y, w, h }) {
          this.ctx.save();
      ...
      getParticle(VCANVASDOM) {
          ...
          const put = () => {
              let arr = this.particleStore[VCANVASDOM.class];
              for (let i = 0; i < arr.length; i++) {
                  arr[i] && arr[i].file && this.ctx.putImageData(arr[i].file, arr[i].x + getRandom(), arr[i].y + getRandom());
              }
          };
      ...
      ;
      // 事件包裹器
      eventWrap({ self, x, y, w, h, click, touchstart, touchmove, touchend }) {
          x /= this.gain;
          y /= this.gain;
          w /= this.gain;
          h /= this.gain;
      ...
      // 能够左右滑动
      canLRSlide(parent) {
          let x = parent.x || 0, y = parent.y || 0, w = parent.computedW || 0, h = parent.computedH || 0, maxMaginLeft = parent.children[0] ? parent.children[0].maginLeft || 0 : 0, minMaginLeft = (document.body.clientWidth - parent.w / this.gain);
          ...
          this.eventStoreArea('touchend', {
              ...
              touchend: () => {
                  ...
                  const run = () => {
                      ...
                      this.draw();
                      if (Math.abs(speed) <= 10 * this.gain)
                          return;
      ...
      ;
      slideTop(child, parent) {
          this.translateSlide(child, parent);
      }
      ;
      translateSlide(child, parent) {
          let timer = 0;
      ...
      ;
      computedText(text = '', fontSize = 28, w = 200) {
